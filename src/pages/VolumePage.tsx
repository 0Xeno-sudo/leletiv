...
export default function VolumePage(){
 ...
 const installVolume=async(file:File,isMask:boolean,modelResult:boolean|'detection'|'archived'=false)=>{
  ...
  else{while(nv.volumes.length)nv.removeVolumeByIndex(0);nv.addVolume(image);const fromImages=image.hdr?.description?.toLowerCase().endsWith(STACK_DESCRIPTION.toLowerCase())??false;setImageStack(fromImages);nv.setIsOrientationTextVisible(!fromImages);setMeta(g);setSource(file.name);setBaseFile(safe);setDemo(false);setStats([]);setMaskName('No region mask');setNote('');setClip(2);nv.setClipPlane([2,0,0]);scanPin.bindVolume(nv,sourceKey);}
 };
 ...
 const exportMeasurements=()=>downloadText('leletiv-region-measurements.csv',csv([[l('Source'),recordText(source)],[l('Mask'),recordText(maskName)],[l('Status'),l('Unvalidated research visualization')],[l('Grid'),meta?.dims.join(' × ')??''],[l('Notes'),note],['Label','Voxels','Volume (mL)'].map(l),...stats.map(s=>[s.label,s.voxels,s.millilitres.toFixed(3)])]));
 return <>
  ...
  <div className="volume-workspace"><section className="volume-main">
   ...
   <button className="button secondary" disabled={!hasMask||busy||stackBusy} onClick={()=>void viewer.current?.volumes[1]?.saveToDisk("leletiv-region.nii")}>{l("Download mask (.nii)")}</button><button className="button secondary" disabled={!baseFile||busy||stackBusy||imageStack} onClick={()=>downloadText('segmentation-request.json',JSON.stringify({schema:'leletiv.segmentation.v1',inputFile:baseFile!.name,geometry:meta,output:'NIfTI-1 integer label mask on the same grid',humanReviewRequired:true,containsImageData:false},null,2))}>{l("Export AI job manifest")}</button>
  </aside></div>
