...
Scan-to-segmentation pipeline|A felvételtől a területek körülhatárolásáig
...
DICOM conversion stays in this browser. Running segmentation sends the four selected sequences only to the inference service on this computer, never to an external AI provider.|A DICOM-fájlok átalakítása a böngészőben történik. Az automatikus körülhatárolás indításakor a négy kiválasztott MR-szekvencia kizárólag az ezen a számítógépen futó helyi feldolgozóhoz kerül, külső MI-szolgáltatóhoz nem.
...
Conversion is not registration or skull stripping. Raw DICOM series may still need preprocessing before the model accepts them.|A fájlátalakítás nem végzi el a felvételek térbeli illesztését és az agyállomány elkülönítését a környező képletektől. A modell használatához további képi előkészítés szükséges lehet.
Prepare four MRI sequences|Négy MR-szekvencia előkészítése
BraTS glioma model · T1c → T1 → T2 → FLAIR · 1 mm isotropic · matching RAS coordinates.|BraTS gliómamodell · T1c → T1 → T2 → FLAIR · Mindhárom irányban 1 mm-es felbontás, azonos képtájolás és térbeli illesztés szükséges.
T1 + contrast|T1 + kontraszt
Choose NIfTI|MR-kép kiválasztása (.nii)
...
Run tumour segmentation|Daganatgyanús területek körülhatárolása
Run synthetic model check|Modellpróba bemutatóadatokkal
The synthetic check runs the actual pretrained weights on a small geometric phantom. It proves execution, not medical accuracy.|A próba a betanított modellt mesterségesen készített próbaképen futtatja. A működést ellenőrzi, nem az orvosi pontosságot.
Start both local services with:|Mindkét helyi szolgáltatás indítása:
Inspect model output|A modell eredményének ellenőrzése
A completed job provides a source-aligned mask, per-region volumes and a reproducible model/input record.|A feldolgozás a felvételen körülhatárolt területeket, azok térfogatát és a számításhoz használt adatok visszakövethető leírását adja.
...
I have visually inspected this research overlay. This is not clinical approval.|Ellenőriztem a felvételen megjelölt területeket. Ez nem klinikai jóváhagyás.
Export model provenance|Feldolgozási adatok letöltése
...
Writing region mask|Területkijelölés létrehozása
...
Synthetic model test phantom|Mesterséges próbakép a modell ellenőrzéséhez
...
Pretrained segmentation · research only|Automatikus körülhatárolás · kizárólag kutatásra
...
No voxels exceeded the model threshold. This does not rule out disease.|A modell a megadott beállításokkal nem jelölt meg gyanús területet. Ez nem zárja ki a betegséget.
...
Use single 3D NIfTI-1 volumes: each dimension at least 16, total at most 12 million voxels.|Egyetlen 3D NIfTI-1 képanyagot adj meg: mindhárom irányban legalább 16, összesen legfeljebb 12 millió térbeli képponttal.
The model requires aligned 1 mm isotropic MRI scans with millimetre units.|A modell térben illesztett, mindhárom irányban 1 mm-es felbontású MR-felvételeket igényel.
...
All sequences must share the same voxel grid and affine coordinates.|Minden MR-szekvencia képmérete, felbontása, térbeli helyzete és tájolása egyezzen meg.
