...
// Keep an existing installation's image store when its identifier is supplied.
const imagingBucket=process.argv[3] || 'leletiv-imaging';
...
await bundle();
const worker=new Miniflare({...convertV4MiniflareOptions({workers:[{name:'leletiv-local',modules:true,script:'export default {fetch(){return new Response("storage")}}',compatibilityDate:'2026-09-13',compatibilityFlags:['nodejs_compat'],d1Databases:{DB:'00000000-0000-0000-0000-000000000001'},r2Buckets:{IMAGING:imagingBucket}}],}),resourcePersistencePath:resolve('.wrangler/state/connected-care')});
await worker.setOptions({...convertV4MiniflareOptions({workers:[{name:'leletiv-local',modules:true,script:await readFile(output+'/worker.js','utf8'),compatibilityDate:'2026-09-13',compatibilityFlags:['nodejs_compat'],d1Databases:{DB:'00000000-0000-0000-0000-000000000001'},r2Buckets:{IMAGING:imagingBucket}}]}),resourcePersistencePath:resolve('.wrangler/state/connected-care')});
...
let pending=Promise.resolve();server.watcher.on('change',path=>{if(/(worker|care-api|scan-api|review-api|demo-api|shared\/).*\.(ts)$/.test(path)){pending=pending.then(async()=>{await bundle();await worker.setOptions({...convertV4MiniflareOptions({workers:[{name:'leletiv-local',modules:true,script:await readFile(output+'/worker.js','utf8'),compatibilityDate:'2026-09-13',compatibilityFlags:['nodejs_compat'],d1Databases:{DB:'00000000-0000-0000-0000-000000000001'},r2Buckets:{IMAGING:imagingBucket}}]}),resourcePersistencePath:resolve('.wrangler/state/connected-care')});}).catch(e=>console.error(e.message));}});
await server.listen();console.log('Local Leletiv ready at http://127.0.0.1:'+port);
